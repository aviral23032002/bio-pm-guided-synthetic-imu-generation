...
def main():
    ...
    parser.add_argument("--real_tokens", default="results_wisdm_v2_6class/token_store.hdf5")
    parser.add_argument("--syn_tokens",  default="synthetic_tokens_v2_6class/synthetic_tokens.hdf5")
    parser.add_argument("--real_waves",  default="preprocessed_wisdm_v2_6class") 
    parser.add_argument("--syn_waves",   default="synthetic_waveforms_v2_6class/synthetic_waveforms.hdf5")
    ...
    # 2. Numerical Metrics
    print(f"  {'Activity':<12} {'Cos Sim':>10} {'RMSE':>10} {'Spec Corr':>10} {'ZCR Err':>10} {'SMA Err':>10}")
    print("  " + "-"*67)
    ...
    for cid in range(6):
        ...
        if m_r.sum() > 5 and m_s.sum() > 5:
            ...
            # Waveform RMSE (between rectified mean profiles)
            w_r_profile = np.abs(rw[m_r]).mean(axis=0)
            w_s_profile = np.abs(sw[m_s]).mean(axis=0)
            rmse = np.sqrt(np.mean((w_r_profile - w_s_profile)**2))
            ...
            print(f"  Class {cid:<7} {t_sim:10.4f} {rmse:10.4f} {s_corr:10.4f} {zcr_err:10.4f} {sma_err:10.4f}")
